...
チャートヘッダーの`CVD SLOPE`は、確定済みCVD系列の傾きをCVD本来の数量単位で表示する。
標準表示は`CVD SLOPE +1.99 BTC/bar (REGRESSION · 20)`の形式とする。

- 数値は成行買い数量と成行売り数量の差を累積したCVDの傾きであり、BTC価格の変化額ではない。
- `BTC/bar`は1本のローソク足当たりのBTC数量を表す。現行1mチャートでは1barは1分である。
- 画面表示だけを小数2桁へ丸め、回帰計算に使う内部値は丸めない。
- `REGRESSION`は窓内の全CVD値に対する最小二乗傾き、`DIFFERENCE`は窓の最初と最後の差を
  区間数で割った傾きとする。
- 標準窓は確定足20本とし、歯車から方式と2〜140本の窓を変更できる。
- 正の傾きは成行買い数量優勢、負の傾きは成行売り数量優勢を表すが、価格方向や売買指示を意味しない。
  価格、Flow Response、CVD divergenceと併読して価格追随、停滞、逆行を観測する。
...
- `CVD SLOPE`が小数2桁と`BTC/bar`単位で表示され、丸め前の長い生小数を画面へ出さないこと。
- `CVD SLOPE`の表示丸め後も、回帰／差分方式、窓、CVD計算値が変更されないこと。
