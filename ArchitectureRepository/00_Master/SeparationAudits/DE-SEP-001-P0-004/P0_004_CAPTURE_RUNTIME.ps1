param(
    [string]$ExternalRoot = 'I:\マイドライブ\DeltaEngine_Preservation\DE-SEP-001-P0-004',
    [string]$VerifyRoot = '',
    [switch]$KeepVerify,
...
$restoreCsvPath = Join-Path $audit 'P0_004_RUNTIME_RESTORE_VERIFICATION.csv'
$activeCsvPath = Join-Path $audit 'P0_004_ACTIVE_SNAPSHOT_FINAL.csv'
$resultPath = Join-Path $audit 'P0_004_RUNTIME_ARCHIVE_RESULT.json'
$progressPath = Join-Path $audit 'P0_004_RUNTIME_CAPTURE_PROGRESS.json'
$duckResultPath = Join-Path $audit 'P0_004_DUCKDB_RESTORE_VERIFICATION.json'
$duckVerifyScript = Join-Path $audit 'P0_004_VERIFY_RESTORED_DUCKDB.py'
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($VerifyRoot)) {
        $VerifyRoot = Join-Path ([IO.Path]::GetTempPath()) ("DE-SEP-001-P0-004_VERIFY_C_$stamp")
    }
    ...
    if ($PreflightOnly) {
        Write-Output ("P0_004_RUNTIME_PREFLIGHT_PASS rows={0} stable={1} active={2} partial={3}" -f $rows.Count, $stableRows.Count, $activeRows.Count, $partialArchive)
        return
    }
    ...
    foreach ($row in $stableRows) {
        ...
        if ($row.relative_path -eq 'data\duckdb\orderflow.duckdb') {
            $row.period_metadata = 'SEE_P0_004_DUCKDB_RESTORE_VERIFICATION.json'
        }
        $processed++
    ...
    Write-Output ("P0_004_RUNTIME_ARCHIVE_PASS path={0} bytes={1} sha256={2}" -f $finalArchive, $finalSize, $finalHash)
} catch {
