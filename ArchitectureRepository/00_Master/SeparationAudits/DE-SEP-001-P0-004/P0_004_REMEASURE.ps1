...
function Get-RelativePath([string]$root, [string]$path) {
    ...
    if ($pathFull.StartsWith($rootFull + '\', [StringComparison]::OrdinalIgnoreCase)) { return $pathFull.Substring($rootFull.Length + 1).Replace('/','') }
...
function Get-Classification([string]$relativePath) {
    ...
    if (@($segments | Where-Object { $_ -eq '__pycache__' -or $_ -eq '.pytest_cache' -or $_ -like '.pytest-*' }).Count -gt 0 -or
...
function Get-GitText([string]$workingDirectory, [string[]]$arguments) {
    ...
    try {
        ...
        $result = @(& git @arguments 2>&1 | ForEach-Object { [string]$_ } | Where-Object { $_ -notmatch '^warning: could not open directory' })
        $exitCode = $LASTEXITCODE
        $ErrorActionPreference = $oldPreference
        if ($exitCode -ne 0) { throw "git $($arguments -join ' ') failed: $result" }
        return $result
    } finally { Set-Location $old }
}

# Immutable legacy evidence is hashed before Run 2 writes continue.
$legacyRows = foreach ($legacyId in @('DE-SEP-001-P0-001', 'DE-SEP-001-P0-002')) {
...
$activity | Export-Csv -LiteralPath (Join-Path $audit 'P0_004_RUNTIME_ACTIVITY_SAMPLE.csv') -NoTypeInformation -Encoding utf8
...
foreach ($path in @($activeSet | Sort-Object)) {
    ...
    if ($path -like 'data\execution_costs\*.csv' -or $path -eq 'data\latency\webapp_brushup.stderr.log') {
        try {
            $prefix = Test-AppendPrefix $fullPath
            $activeEvidence.Add([pscustomobject]@{ relative_path = $path; method = 'SHARED_READ_FIXED_NEWLINE_PREFIX'; cutoff_bytes = $prefix.cutoff_bytes; hash_before = $prefix.prefix_hash_before; hash_after = $prefix.prefix_hash_after; pass = $prefix.pass; detail = '' })
...
$activeEvidence | Export-Csv -LiteralPath (Join-Path $audit 'P0_004_LIVE_SAFE_EVIDENCE.csv') -NoTypeInformation -Encoding utf8
$duckHashAfter = if (Test-Path -LiteralPath $duckPath) { Get-Sha256 $duckPath } else { '' }
if ($duckHashBefore -ne $duckHashAfter) { $gateReasons.Add('DuckDB changed during Run 2 remeasurement') }
...
if ($pq1.count -ne $pq2.count -or $pq1.bytes -ne $pq2.bytes -or $pq1.latest -ne $pq2.latest) { $gateReasons.Add('Parquet set changed during Run 2 remeasurement') }
...
$runtimeRows = foreach ($row in @($hashRows | Where-Object classification -eq 'C')) {
    [pscustomobject]@{
        ...
        capture_method = if ($row.runtime_state -eq 'STABLE_DATA') { 'PRE_POST_HASH_AND_PHYSICAL_ZIP' } elseif ($row.relative_path -eq 'data\execution_costs\status.json') { 'ATOMIC_JSON_SNAPSHOT' } else { 'SHARED_READ_FIXED_NEWLINE_PREFIX' }
        snapshot_size_bytes = ''
        snapshot_sha256 = ''
        pre_sha256 = ''
        post_sha256 = ''
...
$processRows | Export-Csv -LiteralPath (Join-Path $audit 'P0_004_PROCESS_SNAPSHOT.csv') -NoTypeInformation -Encoding utf8
$healthStatus = 'UNAVAILABLE'
$healthError = ''
try {
    $health = Invoke-RestMethod -Uri 'http://127.0.0.1:8080/api/health' -TimeoutSec 10
    $health | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath (Join-Path $audit 'P0_004_HEALTH.json') -Encoding utf8
    if ($null -ne $health.status) { $healthStatus = [string]$health.status }
} catch {
    $healthError = $_.Exception.Message
    @{ status = 'UNAVAILABLE'; error = $healthError } | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $audit 'P0_004_HEALTH.json') -Encoding utf8
}
...
try {
    ...
    [IO.File]::WriteAllLines((Join-Path $audit 'P0_004_PIP_FREEZE.txt'), $freeze, [Text.UTF8Encoding]::new($false))
    $pipFreezeHash = Get-Sha256 (Join-Path $audit 'P0_004_PIP_FREEZE.txt')
} catch { [IO.File]::WriteAllText((Join-Path $audit 'P0_004_PIP_FREEZE.txt'), "UNAVAILABLE: $($_.Exception.Message)", [Text.UTF8Encoding]::new($false)) }
...
$summary | ConvertTo-Json -Depth 20 | Set-Content -LiteralPath (Join-Path $audit 'P0_004_REMEASUREMENT.json') -Encoding utf8

Write-Output ("P0_004_REMEASUREMENT_COMPLETE gate_0a_r_required={0} files={1} runtime={2} active={3}" -f ($gateReasons.Count -gt 0), $hashRows.Count, @($runtimeRows).Count, $activeSet.Count)
