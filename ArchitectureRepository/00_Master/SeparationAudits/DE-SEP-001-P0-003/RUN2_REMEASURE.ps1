...
$activity | Export-Csv -LiteralPath (Join-Path $audit 'RUN2_RUNTIME_ACTIVITY_SAMPLE.csv') -NoTypeInformation -Encoding utf8
...
$activeEvidence | Export-Csv -LiteralPath (Join-Path $audit 'RUN2_LIVE_SAFE_EVIDENCE.csv') -NoTypeInformation -Encoding utf8
...
$processRows | Export-Csv -LiteralPath (Join-Path $audit 'RUN2_PROCESS_SNAPSHOT.csv') -NoTypeInformation -Encoding utf8
...
} catch { [IO.File]::WriteAllText((Join-Path $audit 'RUN2_PIP_FREEZE.txt'), "UNAVAILABLE: $($_.Exception.Message)", [Text.UTF8Encoding]::new($false)) }
...
Write-Output ("RUN2_REMEASUREMENT_COMPLETE gate_0a_r_required={0} files={1} runtime={2} active={3}" -f ($gateReasons.Count -gt 0), $hashRows.Count, @($runtimeRows).Count, $activeSet.Count)
