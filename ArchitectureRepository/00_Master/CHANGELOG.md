...
- 3段チャートヘッダーの`CVD SLOPE`を小数2桁へ丸めた。
- 数量の傾きであることが分かるよう、表示単位を`BTC/bar`へ明示した。
- 表示例を`CVD SLOPE +1.99 BTC/bar (REGRESSION · 20)`へ統一した。

## Interpretation boundary

- CVD SLOPEは成行買い数量と成行売り数量の差を累積したCVDの傾きであり、
  BTC価格の上昇額または下落額ではない。
- 正負は成行数量差の傾向を表し、価格方向、売買シグナル、確率、scoreではない。
- 表示だけを丸め、CVD、回帰／差分計算、標準20本窓、3段チャートの意味は変更していない。

## Verification

- 表示契約テストを追加した。
- 固定チャート表示契約テストを含む対象試験を実行した。
...
- LIVE OBSERVATION右カラムを`238px`へ縮小し、左側のメイン領域を拡大した。
- 3段チャート右側の固定詳細欄を`334px`へ拡大した。
- LIVE OBSERVATIONの主要数値を`14px`、Context値を`11px`、
  Observed Facts本文を`14px`へ拡大した。
- 固定詳細欄の主要数値を`14px`へ統一し、8パターンカード本文を`12px`、
  PRICE／CVD／Delta／OI方向行を`14px`へ拡大した。
- 8パターンの説明を日本語化し、OI評価を可能性、観測事実の順で
  同じカード内へ統合した。
- 選択足詳細へFootprintの`Σ BID`、`Σ ASK`、`Delta`を1行で追加した。

## Fixed

- 固定詳細欄をマウスホイールで縦スクロール可能にした。
- ライブ更新による選択足の再描画時に、詳細欄が先頭へ戻る問題を修正した。
- 別の足へ移動した場合だけ、詳細欄を先頭へ戻すようにした。

## Scope

- Flow Price Response、3段チャートの計算・描画比率、8パターン分類条件、
  OI取得・保存・時刻整列、05M CONTEXT固定帯は変更していない。
- 追加したOI文は観測上の可能性であり、売買シグナル、確率、score、
  自動発注条件ではない。

## Verification

- JavaScript構文確認通過。
- HTML ID `163`件、重複`0`件。
- UI対象テスト`30 passed`。
- WebApp全体`76 passed`。
- 稼働中の05Mがソースと同一SHA-256のHTMLを配信していることを確認した。
