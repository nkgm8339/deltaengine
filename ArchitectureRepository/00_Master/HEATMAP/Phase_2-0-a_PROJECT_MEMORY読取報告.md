# [完了] Phase 2-0-a PROJECT_MEMORY読取
...
- 行数: 439行
- 書込・編集・サービス操作: なし
- 報告書保存先: `ArchitectureRepository/00_Master/HEATMAP/Phase_2-0-a_PROJECT_MEMORY読取報告.md`

## PROJECT_MEMORY.md 全文
...
## 独立観測領域の3段固定と可読性改善（2026-07-25完成）

ユーザーが小さいフォントを無理に読まなくてよいよう、独立観測領域を次の3行へ固定した。

1. `ORDER BOOK | FOOTPRINT`
2. `FLOW EVENTS`
3. `ABSORPTION | IMBALANCE`

画面全体は縦スクロール式とし、見出し15px、主要データ行14px、補助値13pxを基準にした。
ライブ文字列が長くなっても各行の位置と高さは変わらない。多数のImbalanceは3行目を
押し広げず、パネル内部だけをスクロールする。

Flow Price Responseの圧力・継続率・7状態・背景色、3段チャート、8パターン、OI、
各独立指標の計算とpayloadは変更していない。

Edge実ブラウザで1180px／1600px幅、横はみ出し0、縦スクロール、固定位置を確認した。
対象27件、全体 **370 tests passed**。
```
