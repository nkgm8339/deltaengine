(function (global) {
  ...
  class BigTradesContextLink {
    ...
    read() { const value = this.provider() || {}; return { cvd: value.cvd ?? null, delta: value.delta ?? null, volume: value.volume ?? null, flowResponse: value.flowResponse ?? null, absorption: value.absorption ?? null, imbalance: value.imbalance ?? null }; }
  }
  ...
  class BigTradesUI {
    ...
    renderDetail() {
      const zone = this.zoneStore.selected(), detail = zone ? this.zoneStore.details.get(zone.zone_id) : null, eventRow = detail?.origin_event || (zone ? this.eventStore.get(zone.origin_event_id) : null), state = zone ? this.zoneStore.state(zone.zone_id) : {}, snapshots = detail?.horizon_snapshots || (zone ? this.zoneStore.snapshotsFor(zone.zone_id) : []), links = detail?.linked_event_summary || (zone ? this.zoneStore.linksFor(zone.zone_id) : []), interactions = zone ? this.zoneStore.interactionsFor(zone.zone_id) : [], candles = detail?.candle_observations || (zone ? this.zoneStore.candlesFor(zone.zone_id) : []), assessments = zone ? this.zoneStore.assessmentsFor(zone.zone_id) : [], context = this.contextLink.read();
      ...
      this.options.detail.innerHTML = `<div class="bt-selected-title">SELECTED REACTION ZONE</div>` +
        ...
        this.section("CONTEXT", [["CVD", context.cvd ?? no], ["DELTA", context.delta ?? no], ["VOLUME", context.volume ?? no], ["FLOW RESPONSE", context.flowResponse ?? no], ["ABSORPTION", context.absorption ?? no], ["IMBALANCE", context.imbalance ?? no]]) +
  ...
  global.DeltaBigTradesV2 = {
    ...
    BigTradesContextLink, BigTradesUI, ASSESSMENTS,
  };
