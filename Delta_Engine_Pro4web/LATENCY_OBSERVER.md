...
```powershell
python -m tools.observe_hfm_binance --duration-sec 120 --output data_clone/latency/quotes_audit.csv
```
...
sequence, Bid and Ask to `DeltaEngineClone_HFM_quotes_utf8.jsonl` in MT5's common
...
The console report and `data_clone/latency/report.json` contain:
...
written under `data_clone/execution_costs/`. The final report is generated
...
```text
QuoteFileName=DeltaEngineClone_HFM_InfinityX_quotes_utf8.jsonl
```
