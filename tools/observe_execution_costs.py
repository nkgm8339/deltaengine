...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=Path(f"data/execution_costs/quotes_{stamp}.csv"))
    parser.add_argument("--status", type=Path, default=Path("data/execution_costs/status.json"))
    parser.add_argument("--pid-file", type=Path, default=Path("data/execution_costs/observer.pid"))
    parser.add_argument("--cost-config", type=Path, default=Path("config/execution_costs.yaml"))
    parser.add_argument(
        "--report-output",
        type=Path,
        default=Path(f"data/execution_costs/report_{stamp}.json"),
    )
    ...
    if args.hfm_source is None:
        args.hfm_source = [
            ("HFM_CURRENT", default_hfm_path("DeltaEngine_HFM_quotes_utf8.jsonl")),
            ("HFM_INFINITYX", default_hfm_path("DeltaEngine_HFM_InfinityX_quotes_utf8.jsonl")),
        ]
