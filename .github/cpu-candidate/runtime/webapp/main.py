...
from webapp.big_trades_composition import (
    BigTradesComposition,
    build_big_trades_composition,
)
...
async def _big_trades_status_loop(
    broker: PushBroker,
    runtime: Any,
    *,
    interval_sec: float = 0.5,
) -> None:
    """Publish/cache observable Big Trades runtime state without market coupling."""
    ...
    while True:
        ...
        current = (
            ...
            tuple(counters.items()),
        )
        if current != previous:
            await broker.on_big_trades_status(
                snapshot["status"],
                reason=snapshot.get("last_error"),
                counters=counters,
            )
            previous = current
...
async def lifespan(app: FastAPI):
    ...
    if not isinstance(quantity_step, (str, int, Decimal)):
        quantity_step = "0.001"
    try:
        big_trades = build_big_trades_composition(config, broker)
    except Exception as exc:
        # Big Trades fails in isolation; the existing market pipeline still
        # starts and receives no Big Trades runtime.
        logger.exception("Big Trades composition failed")
        big_trades = BigTradesComposition(
        ...
        big_trades.backend.last_error = f"{type(exc).__name__}: {exc}"
        await broker.on_big_trades_status("ERROR", reason=big_trades.backend.last_error)
    else:
        await broker.on_big_trades_status(
            "STARTING" if big_trades.enabled else "DISABLED",
            reason=None if big_trades.enabled else "big_trades.enabled is false",
        )
    big_trades_backend = big_trades.backend
    ...
    if config.replay.enabled:
        pipeline = ReplayPipeline.from_config(
            ...
            big_trades_runtime=big_trades.runtime,
        )
    else:
        pipeline = LivePipeline.from_config(
            config,
            profile,
            big_trades_runtime=big_trades.runtime,
        )
    ...
    app.state.big_trades = big_trades
    ...
    big_trades_batcher_task = (
        asyncio.create_task(big_trades.batcher.run())
        if big_trades.batcher is not None
        else None
    )
    big_trades_status_task = (
        asyncio.create_task(_big_trades_status_loop(broker, big_trades.runtime))
        if big_trades.runtime is not None
        else None
    )
    ...
    if big_trades_batcher_task is not None:
        tasks.append(big_trades_batcher_task)
    if big_trades_status_task is not None:
        tasks.append(big_trades_status_task)
    ...
    try:
        ...
        with contextlib.suppress(Exception):
            await asyncio.to_thread(big_trades.close)
        with contextlib.suppress(Exception):
            await broker.close()
        if hook_capture is not None:
            with contextlib.suppress(Exception):
...
async def api_stats(request: Request):
    ...
    timing_observer = getattr(pipeline, "pipeline_timing", None)
    timing_snapshot = getattr(timing_observer, "stats_snapshot", None)
    if callable(timing_snapshot):
        timing_payload = timing_snapshot()
        if isinstance(timing_payload, dict):
            stats["pipeline_timing"] = timing_payload
    storage = getattr(pipeline, "storage_writer", None)
    if storage is not None:
