...
@dataclass(frozen=True)
class BoundaryActivationOutcome:
    activated: bool
    failure_code: Optional[str] = None
    storage_degraded: bool = False
    pointer_cache_degraded: bool = False


class LiveActivationSettingsResolver:
    """Artifact-ledger resolver for the enabled production composition.

    The activation artifact is authoritative. DuckDB settings/calibration/
    activation rows are durable mirrors, and the first source trade cannot
    reach aggregation until PRODUCTION_INITIAL is committed.
    """
    ...
    def __init__(
        ...
        *,
        ...
        storage_writer: BigTradesBackgroundStorageWriter,
        initial_settings: SettingsVersion,
        quantity_step: Decimal | str,
        calibration_schedule: CalibrationSchedule | str,
        activation_policy: CalibrationActivationPolicy | str,
        commit_timeout_sec: float = 5.0,
    ) -> None:
        if commit_timeout_sec <= 0:
            raise ValueError("commit_timeout_sec must be positive")
        self.repository = repository
        self.storage_writer = storage_writer
        self.initial_settings = initial_settings
        self.quantity_step = Decimal(str(quantity_step))
        self.calibration_schedule = CalibrationSchedule(calibration_schedule)
        self.activation_policy = CalibrationActivationPolicy(activation_policy)
        self.commit_timeout_sec = float(commit_timeout_sec)
        self.settings = initial_settings
        self.activation: Optional[ActivationArtifact] = None
        self.calibration: Optional[CalibrationArtifact] = None
        self._snapshot: Optional[BigTradesSettingsSnapshot] = None
        self._automatic: Optional[AutomaticSizeFilter] = None
        self.pointer_cache_degraded = False
        repository.write_settings_version(initial_settings)
        matching = [
            item
            for item in repository.load_activations()
            if item.symbol == initial_settings.symbol
            and item.venue == initial_settings.venue
            and item.input_mode == initial_settings.input_mode
            and item.logic_version == initial_settings.logic_version
        ]
        if matching:
            latest = matching[-1]
            self._install(
                repository.load_settings_version(latest.settings_id),
                latest,
            )

    @property
    def active_settings(self) -> SettingsVersion:
        return self.settings

    def _wait_for_commit(self, future: CommitFuture) -> CommitAck:
        ack = future.result(timeout=self.commit_timeout_sec)
        if not ack.success or not ack.duckdb_committed:
            raise RuntimeError(ack.error or ack.error_code or "BIG_TRADES_STORAGE_COMMIT_FAILED")
        return ack

    def _load_calibration(
        self, settings: SettingsVersion
    ) -> Optional[CalibrationArtifact]:
        if settings.calibration_id is None:
            return None
        calibration = self.repository.load_calibration(settings.calibration_id)
    ...
    def _install(
        ...
        if activation.settings_id != settings.settings_id:
            raise ActivationHistoryError("activation/settings mismatch")
        if activation.calibration_id != settings.calibration_id:
            raise ActivationHistoryError("activation/settings calibration mismatch")
        calibration = self._load_calibration(settings)
        self.settings = settings
        self.activation = activation
        self.calibration = calibration
        self._snapshot = settings.to_runtime_snapshot(
    ...
    def _commit_initial(self, trade: BigTradeFill) -> None:
        request = SettingsRequest.create(
            self.initial_settings,
            requested_at_utc=trade.event_time,
            previous_settings_id=None,
            request_source=SettingsRequestSource.CONFIG,
            request_status=SettingsRequestStatus.ACTIVATED,
        )
        self.repository.write_settings_request_history(request)
        self._wait_for_commit(
            self.storage_writer.submit_settings(request, self.initial_settings)
        )
        activation = ActivationArtifact.create(
            symbol=trade.symbol,
            venue=trade.venue,
            effective_from_event_time=trade.event_time,
            effective_from_trade_id=trade.trade_id,
            settings_id=self.initial_settings.settings_id,
            calibration_id=self.initial_settings.calibration_id,
            activation_reason=ActivationReason.PRODUCTION_INITIAL,
            activation_policy=self.activation_policy,
            requested_at_utc=trade.event_time,
        )
        commit = self.repository.commit_activation(
            activation,
            expected_quantity_step=self.quantity_step,
        )
        self.pointer_cache_degraded = not commit.pointer_cache_updated
        self._install(self.initial_settings, activation)
        self._wait_for_commit(self.storage_writer.submit_activation(activation))

    def resolve(self, trade: BigTradeFill) -> ResolvedRuntimeSettings:
        if trade.symbol != self.settings.symbol or trade.venue != self.settings.venue:
            raise ValueError("trade identity does not match live activation")
        if self.activation is None:
            self._commit_initial(trade)
        assert self.activation is not None and self._snapshot is not None
    ...
    def automatic_filter_for(
        ...
        if self._snapshot is None or snapshot.settings_id != self._snapshot.settings_id:
            return None
        return self._automatic

    @staticmethod
    def _calibration_only_request(
        active: SettingsVersion,
        pending: SettingsVersion,
    ) -> bool:
        return (
            active.filter_mode is FilterMode.AUTOMATIC
            and pending.filter_mode is FilterMode.AUTOMATIC
            and active.calibration_id != pending.calibration_id
            and active.manual_min_quantity == pending.manual_min_quantity
            and active.manual_max_quantity == pending.manual_max_quantity
            and active.automatic_intensity == pending.automatic_intensity
            and active.side_filter == pending.side_filter
            and active.marker_price_mode == pending.marker_price_mode
        )

    def activate_at_source_boundary(
        self,
        transition: Any,
        boundary_trade: BigTradeFill,
    ) -> BoundaryActivationOutcome:
        if self.activation is None:
            raise ActivationHistoryError("source boundary preceded initial activation")
        pending = self.repository.load_pending_settings(
            symbol=self.settings.symbol,
            venue=self.settings.venue,
            input_mode=self.settings.input_mode,
        )
        pending_request = pending[0] if pending is not None else None
        pending_version = pending[1] if pending is not None else None

        # Reconcile a request whose activation artifact committed before the
        # process stopped but whose terminal pointer cleanup did not finish.
        if (
            pending_request is not None
            and pending_version is not None
            and pending_version.settings_id == self.settings.settings_id
            and self.activation.requested_at_utc >= pending_request.requested_at_utc
        ):
            terminal = self.repository.complete_pending_settings(
                pending_request, pending_version
            )
            self._wait_for_commit(
                self.storage_writer.submit_settings(terminal, pending_version)
            )
            pending_request = None
            pending_version = None

        pending_settings = pending_version
        pending_user_calibration_id: Optional[str] = None
        if pending_version is not None and self._calibration_only_request(
            self.settings, pending_version
        ):
            pending_settings = None
            pending_user_calibration_id = pending_version.calibration_id

        execution = execute_scheduled_calibration(
            self.repository,
            transition=transition,
            schedule=self.calibration_schedule,
        ...
        resolved = execution.resolved_activation if execution is not None else None
        failure_code = None
        if execution is not None and execution.run.result in {
            ScheduleRunResult.VALIDATION_FAILED,
            ScheduleRunResult.STORAGE_FAILED,
        }:
            failure_code = execution.run.failure_code or execution.run.result.value

        # A scheduled candidate failure must not silently discard an explicit
        # user request. User requests retain boundary priority.
        if resolved is None and pending_version is not None:
            resolved = resolve_activation_at_boundary(
                active_settings=self.settings,
                boundary_trade=boundary_trade,
                policy=self.activation_policy,
                requested_at_utc=pending_request.requested_at_utc,
                pending_settings=pending_settings,
                pending_user_calibration_id=pending_user_calibration_id,
            )
            if resolved is not None:
                self.repository.write_settings_version(resolved.settings)
                commit = self.repository.commit_activation(
                    resolved.activation,
                    expected_quantity_step=self.quantity_step,
                )
        ...
        previous_settings_id = self.settings.settings_id
        if resolved is not None:
            # Once the artifact rename commits it is authoritative, even if a
            # later DuckDB mirror write reports degradation.
            self._install(resolved.settings, resolved.activation)
        if execution is not None and execution.candidate is not None:
            self._wait_for_commit(
                self.storage_writer.submit_calibration(execution.candidate)
            )
        if resolved is None:
            return BoundaryActivationOutcome(
                False,
                failure_code=failure_code,
                pointer_cache_degraded=self.pointer_cache_degraded,
            )

        terminal_request: Optional[SettingsRequest] = None
        if pending_request is not None and pending_version is not None:
            terminal_request = self.repository.complete_pending_settings(
                pending_request, pending_version
            )
            self._wait_for_commit(
                self.storage_writer.submit_settings(terminal_request, pending_version)
            )
        if pending_version is None or pending_version.settings_id != resolved.settings.settings_id:
            scheduled_request = SettingsRequest.create(
                resolved.settings,
            ...
            self.repository.write_settings_request_history(scheduled_request)
            self._wait_for_commit(
                self.storage_writer.submit_settings(scheduled_request, resolved.settings)
            )
        self._wait_for_commit(
            self.storage_writer.submit_activation(resolved.activation)
        )
        return BoundaryActivationOutcome(
            True,
            failure_code=failure_code,
...
class BigTradesRuntimeV2:
    ...
    def __init__(
        ...
        *,
        ...
        max_fills_per_cluster: int = 10_000,
        ...
        if (
            zone_capacity < 1
            or recent_capacity < 1
            or link_tolerance_ticks < 0
        ...
        self.aggregator = ExecutionClusterAggregator(
            max_fills_per_cluster=max_fills_per_cluster
        )
    ...
    def _process_ordered(self, trade: BigTradeFill) -> None:
        ...
        if transition is not None:
            self._complete_source_confirmed_session(transition, boundary_trade=trade)
        ...
        if getattr(self.settings_resolver, "pointer_cache_degraded", False):
            self.status = BigTradesRuntimeStatus.DEGRADED_POINTER_CACHE
        else:
    ...
    def _set_ready_status(self, resolved: ResolvedRuntimeSettings) -> None:
        if self.status in {
            BigTradesRuntimeStatus.CALIBRATION_ACTIVATION_FAILED,
            BigTradesRuntimeStatus.DEGRADED_SOURCE_GAP,
            BigTradesRuntimeStatus.DEGRADED_STORAGE,
            BigTradesRuntimeStatus.DEGRADED_POINTER_CACHE,
    ...
    def _handle_finalized_cluster(
        ...
        *,
        ...
        pending = _PendingOrigin(token, future, batch, event, zone, records)
    ...
    def _complete_source_confirmed_session(
        self,
        transition: Any,
        *,
        boundary_trade: BigTradeFill,
        ...
        for cluster in self.aggregator.flush(ClusterCloseReason.SESSION_CHANGED):
            self._handle_finalized_cluster(cluster, post_trade=None)
        # A cluster finalized by the boundary may have created a new origin
        # whose close/checkpoint writes are still buffered behind its ACK.
        # Settle that origin before creating the old-session terminal facts.
        boundary_storage_ok = self._settle_boundary_writes()
        ...
        # Dependents buffered behind a just-created origin are submitted while
        # ACKs are drained.  A second barrier proves all old-zone terminal
        # facts are durable before session stats and any activation work.
        boundary_storage_ok = self._settle_boundary_writes() and boundary_storage_ok
        ...
        # The writer is FIFO. Waiting for the stats acknowledgement therefore
        # proves that zone close/checkpoint writes submitted before it are also
        # durable before scheduled calibration or activation begins.
        try:
            stats_ack = future.result(timeout=5.0)
        except TimeoutError:
            stats_ack = None
            self.status = BigTradesRuntimeStatus.DEGRADED_STORAGE
            self.last_error = "SESSION_STATS_COMMIT_TIMEOUT"
        self.drain_commit_acks()

        if (
            stats_ack is not None
            and stats_ack.success
            and boundary_storage_ok
            and not self._halted
        ):
            activate = getattr(
                self.settings_resolver,
                "activate_at_source_boundary",
                None,
            )
            if callable(activate):
                try:
                    outcome = activate(transition, boundary_trade)
                except Exception as exc:  # noqa: BLE001 - isolate existing pipeline
                    self.status = BigTradesRuntimeStatus.CALIBRATION_ACTIVATION_FAILED
                    self.last_error = f"{type(exc).__name__}: {exc}"
                    logger.exception("Big Trades source-boundary activation failed")
                else:
                    if outcome.storage_degraded:
                        self.status = BigTradesRuntimeStatus.DEGRADED_STORAGE
                    elif outcome.pointer_cache_degraded:
                        self.status = BigTradesRuntimeStatus.DEGRADED_POINTER_CACHE
                    elif outcome.failure_code:
                        self.status = BigTradesRuntimeStatus.CALIBRATION_ACTIVATION_FAILED
                    if outcome.failure_code:
                        self.last_error = outcome.failure_code
        elif stats_ack is not None and not stats_ack.success:
            self.status = BigTradesRuntimeStatus.DEGRADED_STORAGE
            self.last_error = stats_ack.error or stats_ack.error_code

        # ACK cleanup may already have removed closed observers.  Replacing the
        # index avoids leaving a stale boundary entry whose observer no longer
        # exists when the first trade of the new session is processed.
        self.zone_index = ReactionZoneBoundaryIndex()
        self.observers.clear()
        self.events_by_zone.clear()
        self._committed_zones.clear()
        self._closed_zone_cleanup_pending.clear()
    ...
    def _settle_boundary_writes(self, *, timeout_seconds: float = 5.0) -> bool:
        """Wait for all writes already implied by the old source session.

        ACK handling can enqueue dependents after an origin becomes durable, so
        this is intentionally a bounded loop rather than a single queue join.
        It runs only at a source-confirmed session boundary, never per trade.
        """

        deadline = time.monotonic() + timeout_seconds
        while self._pending_origin_by_token or self._pending_updates:
            futures = tuple(
                pending.future for pending in self._pending_origin_by_token.values()
            ) + tuple(pending.future for pending in self._pending_updates.values())
            try:
                ...
                self.status = BigTradesRuntimeStatus.DEGRADED_STORAGE
                self.last_error = "SESSION_BOUNDARY_COMMIT_TIMEOUT"
                return False
            self.drain_commit_acks()
        return not self._halted and self.status not in {
            BigTradesRuntimeStatus.DEGRADED_STORAGE,
            BigTradesRuntimeStatus.ERROR,
        }
    ...
    def _register_update(
        ...
        self._pending_updates[token] = _PendingUpdate(
            token, future, kind, records, zone_ids, on_success
        )
        return token
