...
class BigTradesArtifactRepository:
    ...
    def load_pending_settings(
        ...
        *,
        ...
        path = self._pending_settings_path(
            symbol=symbol,
            venue=venue,
            input_mode=input_mode,
        )
        if not path.exists():
            return None
        request = SettingsRequest.from_artifact(self._read_json(path))
        # A crash after the terminal status replace but before pointer cleanup
        # must not reactivate an already-consumed request at a later boundary.
    ...
    def _pending_settings_path(
        ...
        *,
        ...
        return self.settings_root / "pending" / f"{key}.json"
    ...
    def complete_pending_settings(
        self,
        request: SettingsRequest,
        version: SettingsVersion,
        *,
        status: SettingsRequestStatus | str = SettingsRequestStatus.ACTIVATED,
    ) -> SettingsRequest:
        """Durably terminalize one pending pointer after activation commit.

        The terminal artifact replaces the pending pointer before unlink.  If
        the process stops between those operations, ``load_pending_settings``
        treats the terminal pointer as consumed instead of activating it again.
        """

        terminal_status = SettingsRequestStatus(status)
        if terminal_status is SettingsRequestStatus.PENDING:
            raise ValueError("completed pending settings require terminal status")
        if request.request_status is not SettingsRequestStatus.PENDING:
            raise ArtifactIntegrityError("settings request is not PENDING")
        if request.settings_id != version.settings_id:
            raise ArtifactIntegrityError("pending request settings mismatch")
        path = self._pending_settings_path(
            symbol=version.symbol,
        ...
        if path.exists():
            current = SettingsRequest.from_artifact(self._read_json(path))
            if current.request_status is SettingsRequestStatus.PENDING:
                if current.request_id != request.request_id:
                    raise ArtifactIntegrityError("pending settings pointer changed")
            elif current.settings_id != request.settings_id:
                raise ArtifactIntegrityError("terminal settings pointer changed")
        terminal = request.with_status(terminal_status)
        self.write_settings_request_history(terminal)
        self._atomic_replace(path, terminal.to_artifact())
        stored = SettingsRequest.from_artifact(self._read_json(path))
        if stored.request_id != terminal.request_id:
            raise ArtifactIntegrityError("terminal pending pointer read-back mismatch")
        try:
            path.unlink()
        except FileNotFoundError:
            pass
