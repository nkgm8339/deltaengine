...
from ..monitor.pipeline_timing import DurationWindow
...
class BoundedEventQueue:
    ...
    def __init__(
        ...
        self._residence_ms = DurationWindow()
    ...
    def _record_residence(self) -> None:
        if not self._enqueued_at:
            return
        enqueued_at = self._enqueued_at.popleft()
        self._residence_ms.observe_ms(
            max(0.0, time.monotonic() - enqueued_at) * 1_000.0
        )
    ...
    def stats_snapshot(self) -> dict[str, Any]:
        return {
            ...
            "residence_ms": self._residence_ms.stats_snapshot(),
        }
