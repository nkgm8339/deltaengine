...
from .monitor.pipeline_timing import PipelineTimingObserver, observe_event_loop_lag
...
class LivePipeline:
    ...
    def __init__(
        ...
        *,
        ...
        self.pipeline_timing = PipelineTimingObserver()
    ...
    async def run_async(
        ...
        *,
        ...
        def call_callback(callback: Callable[..., Any], *args: Any) -> Any:
            with timing.measure("callback"):
                return callback(*args)

        timed_absorption_callback = (
            None
            if self.on_absorption_state is None
            else lambda *args: call_callback(self.on_absorption_state, *args)
        )
        timed_webapp_flow_callback = (
            None
            if self.on_webapp_flow_event is None
            else lambda *args: call_callback(self.on_webapp_flow_event, *args)
        )
        timed_native_candle_callback = (
            None
            if self.on_native_candle is None
            else lambda *args: call_callback(self.on_native_candle, *args)
        )
        ...
        native_coordinator = NativeExecutionCoordinator(
            self.symbol,
            on_candle=timed_native_candle_callback,
        )
        ...
        def _emit_flow(evt: FlowEvent) -> None:
            ...
            if self.on_flow_event is not None:
                call_callback(self.on_flow_event, evt)
        ...
        def handle(normalized) -> None:
            ...
            if self.on_accepted_trade is not None:
                try:
                    call_callback(self.on_accepted_trade, normalized)
                except Exception:
                    logger.exception(
                        "accepted-trade observer failed; live analysis continues"
                    )
            with timing.measure("storage"):
                storage.add_trade(trade_to_row(normalized))
            native_coordinator.process(normalized, storage)
            self._last_event_time = normalized.event_time
            producer.observe_trade(normalized)
            if flow_response_detector is not None and flow_response_tracker is not None:
                ...
                if snapshots:
                    ...
                    for event in events:
                        storage.add_flow_response_event(flow_response_event_to_row(event))
                    if self.on_flow_response is not None:
                        call_callback(self.on_flow_response, snapshots)
                outcomes = flow_response_tracker.observe_trade(normalized)
                self.flow_response_outcomes.extend(outcomes)
                for outcome in outcomes:
                    storage.add_flow_response_outcome(flow_response_outcome_to_row(outcome))
            if self.on_trade is not None:
                call_callback(self.on_trade, normalized)
            cvd_result = cvd.process(normalized)
            if cvd_result.update is not None:
                producer.observe_cvd(cvd_result.update)
            if self.big_trades_runtime is not None:
            ...
            if self.on_webapp_flow_event is not None and absorption.events_detected > prev_abs:
                if ar is not None:
                    ...
                    call_callback(self.on_webapp_flow_event, PushFlowEvent(
            ...
            if cvd_result.closed_candle is not None:
                ...
                if fp_closed is None:
                    ...
                    if self.on_candle is not None:
                        call_callback(self.on_candle, cvd_result.closed_candle)
                else:
                    ...
                    bar_close = _evaluate_and_store(
                        ...
                        on_webapp_flow_event=timed_webapp_flow_callback,
                    ...
                    if bar_close.analysis_result is not None and self.on_analysis is not None:
                        call_callback(self.on_analysis, bar_close.analysis_result)
        ...
        timing_task = asyncio.create_task(observe_event_loop_lag(timing))
        ...
        try:
            while True:
                ...
                kind = normalizer.classify_raw(raw)
                if kind == "liquidation":
                    with timing.measure("liquidation"):
                        try:
                            ...
                            if self.on_liquidation is not None:
                                call_callback(self.on_liquidation, liq_evt)
                        except NormalizationError as exc:
                            logger.warning("liquidation normalization failed: %s", exc)
                elif kind == "depth":
                    with timing.measure("depth"):
                    ...
                    with timing.measure("normalize"):
                        normalized_batch = normalizer.process(raw)
                    for normalized in normalized_batch:
                        with timing.measure("trade"):
                            handle(normalized)
                    trades_in += 1
                with timing.measure("storage"):
                    storage.tick()
            ...
            while not norm_q.empty():
                ...
                if kind == "liquidation":
                    ...
                    with timing.measure("normalize"):
                        normalized_batch = normalizer.process(pending)
                    for normalized in normalized_batch:
                        with timing.measure("trade"):
                            handle(normalized)
            drain_snapshot_results()
            with timing.measure("normalize"):
                normalized_batch = normalizer.flush()
            for normalized in normalized_batch:
                with timing.measure("trade"):
                    handle(normalized)
            _finish_big_trades_runtime(self.big_trades_runtime)
